...
public class AESCipher
{
    ...
    public static byte[] testKey_s = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };

    public static byte[,] testInput_s = 
    {
        {0x32, 0x88, 0x31, 0xe0},
        {0x43, 0x5a, 0x31, 0x37},
        {0xf6, 0x30, 0x98, 0x07},
    ...
    public static byte[] testKey2 = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };

    public static byte[] testKey3 = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17 };

    public static byte[] test_plaintext = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff };
    

    public static byte[] getCipherKey(int Nk)
    {
        int len = 0;
    ...
    public static byte[] MessageInBinary(string message, Encoding encoding)
    {
    ...
    public static byte[,] BlockToState(byte[] block)
    {
    ...
    public static byte[] StateToBlock(byte[,] state)
    {
        byte[] retValue = {
    ...
    public static byte GetSubstituteByte(byte s)
    {
    ...
    //using a nonlinear byte substitution table (S-box)
    ...
    public static void SubBytes(ref byte[,] state)  
    {
        for(int i = 0; i < 4; i++)
        {
            for(int j = 0; j < 4; j++)
            {
                state[i, j] = GetSubstituteByte(state[i, j]);
            }
        }
    }

    public static byte GF256multiplication(byte a, byte b)
    {
        byte retValue = 0;
        byte t;
    ...
    public static void MixColumns(ref byte[,] state)
    {
        ...
        for(int c = 0; c < 4; c++)
        {
            helper_array[0] = (GF256multiplication(0x02, state[0, c])) ^ (GF256multiplication(0x03, state[1, c])) ^ (state[2, c]) ^ (state[3, c]);
            helper_array[1] = (state[0, c]) ^ (GF256multiplication(0x02, state[1, c])) ^ (GF256multiplication(0x03, state[2, c])) ^ (state[3, c]);
            helper_array[2] = (state[0, c]) ^ (state[1, c]) ^ (GF256multiplication(0x02, state[2, c])) ^ (GF256multiplication(0x03, state[3, c]));
            helper_array[3] = (GF256multiplication(0x03, state[0, c])) ^ (state[1, c]) ^ (state[2, c]) ^ (GF256multiplication(0x02, state[3, c]));
    ...
    public static void AddRoundKey(ref byte[,] state, word[] ks, int round)
    {
        int Nb = 4;
        int[] helper_array = new int[4];
        for (int c = 0; c < 4; c++)
    ...
    public static word[] KeyExpansion(byte[] K)
    {
        int Nb = 4;
        int Nk = K.Length / 4;
        int Nr = Nb + Nk + 2;
    ...
    public static byte[,] Cipher(byte[,] input, word[] KeyScheadule, int Nk)
    {
        byte[,] state = input;
        int Nb = 4;
        int lastRound = 0;
        AddRoundKey(ref state, KeyScheadule, 0);
        ...
        for (int round = 1; round < (Nb + Nk + 2); round++)
        {
            ...
            AddRoundKey(ref state, KeyScheadule, round);
            lastRound = round;
        }

        SubBytes(ref state);
        ShiftRows(ref state);
        AddRoundKey(ref state, KeyScheadule, lastRound + 1);
    ...
    {
        word[] KeyScheadule = KeyExpansion(testKey3);

        byte[,] complete1 = Cipher(BlockToState(test_plaintext), KeyScheadule, 6);

        byte[] output = StateToBlock(complete1);
        Console.Write(BitConverter.ToString(output));
    }
}